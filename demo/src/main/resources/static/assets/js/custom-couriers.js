...
/* Pagination */
	...
	var url = HOST_URL + "/courier/couriers?currentPage=";
	$.ajax({
		url: HOST_URL + "/courier/couriers/",
		type: "GET",
...
/* Pagination */
	$.ajax({
		url: HOST_URL + "/courier/couriers?currentPage="+currentPage,
		type: "GET",
		success:function(data) {
			data = data.content;
			var counter = 0;
			data.forEach(function(stockmanData,index){
				counter++;
								
				var name=("<tr id='"+stockmanData._id+"'><td class='text-center'><span class='text-dark-75 font-weight-bolder d-block font-size-lg' data-toggle='modal' data-target='.bd-example-modal-lg' style='cursor: pointer;'>"+stockmanData.firstName+" " +stockmanData.lastName+ "</span></td>");
		        var categoryName=("<td class='text-center'><span class='text-dark-75 font-weight-bolder d-block font-size-lg'>"+stockmanData.email+"</span></td>");
		        var description=("<td class='text-center'><span class='text-dark-75 font-weight-bolder d-block font-size-lg'>"+stockmanData.username+"</span></td>");
		        
		        if(stockmanData.status == true){
		        	statusPrint = "checked='checked'";
		        }else{
		        	statusPrint = "";
		        }
		        var status=("<td class='text-center'>"+"<div class=''><div class='col-3'><span class='switch switch-outline switch-icon switch-success'><label><input class=' statusUpdate' id='"+stockmanData._id+"' type='checkbox' "+statusPrint+" name='select'/><span></span></label></span></div>"+"</td>");
				...
				if(stockmanData.status == true){
 				...
 				var options=("<td class='text-center'>"+"<a href='courierEdit?id="+stockmanData._id+"'><div class='btn btn-icon btn-dark btn-hover-dark btn-sm categoryEdit' id='"+stockmanData._id+"'><span class='svg-icon svg-icon-md svg-icon-primary'><div class='fa fa-edit'></div></span></div></a>"+"<div class='btn btn-icon btn-danger btn-hover-dark btn-sm subCategoryDelete' id='"+stockmanData._id+"'><span class='svg-icon svg-icon-md svg-icon-primary'><div class='fa fa-trash'></div></span></div>"+"</td></tr>");
	        
				$("#printCourier").append(name+categoryName+description+status+options);

	    	});
	    	
			$("#kt_subheader_total").html("("+counter+ " Kurye)");
...
/*--------------------------Status Update--------------------------*/
			    $('.statusUpdate').click(function (event) {
			        ...
			        var statusUpdate = ($(this).is(':checked')) ? true : false;
					        ...
					        $.ajax({
					        	url: HOST_URL + "/courier/status/"+id,
								type: "PUT",
					            data: ser_data,
...
/*--------------------------SubCategory Delete--------------------------*/
			...
			function subCategoryDelete(silinecekId){
				$(function() {
					...
					Swal.fire({
					title: "Kuryeyi silmek istediğinizden emin misiniz?",
					...
					if (result.value) {
						$.ajax({
						url: HOST_URL + "/courier/courier/"+silinecekId+"/",
						type: "DELETE",
					    contentType: "application/json",
						success: function(sonuc) {
						  $("#"+silinecekId).remove();
						  $("#kt_subheader_total").html("("+(counter-1)+ " Kurye)");
						}
						});
					
					    Swal.fire({
							position: "top-right",
					        title: "Kurye başarıyla silindi!",
					        icon: "success",
...
/*--------------------------SubCategory Add--------------------------*/
			    ...
			    $('#subCategorySave').click(function (event) {
			        ...
			        var categoryName = $(".printCourier option:selected" ).text();
			        var categoryId = $(".printCourier option:selected" ).val();
			        ...
			        $.ajax({
			            ...
			            success: function (result) {
			                toastr.success('Alt Kategori Eklendi.', 'İşlem Başarılı', {"progressBar": true,positionClass: "toast-bottom-right",timeOut: 1500})
									setTimeout(function(){   
							        window.location.assign("/subCategories");
							        }, 1500);
			            },
			            error: function () {
					        if(title == "" || title == null){
