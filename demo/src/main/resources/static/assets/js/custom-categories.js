...
/* Pagination */
	...
	var url = HOST_URL + "/dashboard/categories?currentPage=";
	$.ajax({
...
/* Pagination */
	$.ajax({
		url: HOST_URL + "/rest/categories?currentPage="+currentPage,
		type: "GET",
		success:function(data) {
			data = data.content;
			var counter = 0;
...
/*--------------------------Status Update--------------------------*/
			    $('.statusUpdate').click(function (event) {
			        ...
			        $.ajax({
			        	url: HOST_URL + "/rest/category/updateStatus/"+id,
						type: "PUT",
			            data: ser_data,
...
/*--------------------------Category Delete--------------------------*/
			...
			function categoryDelete(silinecekId){
				$(function() {
					...
					if (result.value) {
						$.ajax({
						url: HOST_URL + "/rest/category/"+silinecekId+"/",
						type: "DELETE",
					    contentType: "application/json",
