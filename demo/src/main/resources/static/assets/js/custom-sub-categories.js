...
/* Pagination */
	...
	var url = HOST_URL + "/dashboard/subCategories?currentPage=";
	$.ajax({
		url: HOST_URL + "/rest/subCategories/",
		type: "GET",
...
/* Pagination */
	$.ajax({
		url: HOST_URL + "/rest/subCategories?currentPage="+currentPage,
		type: "GET",
		success:function(data) {
			data = data.content;
			var counter = 0;
			data.forEach(function(subCategoryData,index){
				...
				var name=("<tr id='"+subCategoryData._id+"'><td class='text-center'><span class='text-dark-75 font-weight-bolder d-block font-size-lg' data-toggle='modal' data-target='.bd-example-modal-lg' style='cursor: pointer;'>"+subCategoryData.title+"</span></td>");
		        var categoryName=("<td class='text-center'><span class='text-dark-75 font-weight-bolder d-block font-size-lg' data-toggle='modal' data-target='.bd-example-modal-lg' style='cursor: pointer;'>"+subCategoryData.categoryName+"</span></td>");
		        var description=("<td class='text-center'><span class='text-dark-75 font-weight-bolder d-block font-size-lg'>"+subCategoryData.description+"</span></td>");
		        
		        if(subCategoryData.status == true){
		        	statusPrint = "checked='checked'";
		        }else{
		        	statusPrint = "";
		        }
		        var status=("<td class='text-center'>"+"<div class=''><div class='col-3'><span class='switch switch-outline switch-icon switch-success'><label><input class=' statusUpdate' id='"+subCategoryData._id+"' type='checkbox' "+statusPrint+" name='select'/><span></span></label></span></div>"+"</td>");
...
/*--------------------------SubCategory Delete--------------------------*/
			...
			function subCategoryDelete(silinecekId){
				$(function() {
					...
					Swal.fire({
					title: "Alt Kategoriyi silmek istediğinizden emin misiniz?",
					...
					if (result.value) {
						$.ajax({
						url: HOST_URL + "/rest/subCategory/"+silinecekId+"/",
						type: "DELETE",
					    contentType: "application/json",
					    ...
					    Swal.fire({
							position: "top-right",
					        title: "Alt Kategori başarıyla silindi!",
					        icon: "success",
