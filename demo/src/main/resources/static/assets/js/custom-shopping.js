	...
	$.ajax({
		...
		success:function(data) {
			...
			$(".categorySelect").click( function () {
				...
				$(".deleted").remove();
				$.ajax({
				url: HOST_URL + "/rest/findProductByCategory?categoryId="+id,
				type: "GET",
				success:function(data) {
					data = data;
					...
					data.forEach(function(categoryData,index){
						var imageUrl = "";
							...
							if(categoryData.imageUrl == "" || categoryData.imageUrl == null){
								if(categoryData.productImages != "" && categoryData.productImages != null){
									imageUrl = categoryData.productImages[0].imageUrl;
								}else{
											...
											'<div class="card-body p-0">'+
												'<div class="overlay">'+
													'<div class="overlay-wrapper rounded bg-light text-center">'+
														'<img src="'+imageUrl+'" alt="" class="mw-100 w-200px" />'+
													'</div>'+
