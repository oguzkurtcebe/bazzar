   		...
   		document.getElementById("enterSearch").click();
			...
			$(".searchProduct").click(function(){
				...
				if(titleSearch == "" || titleSearch == null){
					url = _URL + "/delivery/products/?currentPage="+currentPage;
					$("#printPagination").show();
				}else{
					$("#printPagination").hide();
					$("#printProduct").html("");
					url = HOST_URL + "/delivery/findDeliveryProduct?title="+titleSearch;
				}
				$.ajax({
					url: url,
					type: "GET",
					success: function(data){
						$("#pageLoader").hide();
						if(url == HOST_URL + "/delivery/products/?currentPage="+currentPage){
						...
						data.forEach(function(productData,index){
							...
							$("#printProduct").append(image+name+price+category+status+options);
			
				    	});
				    	
						if(url == HOST_URL + "/delivery/users?currentPage="+currentPage+"&fn=&ln="){//delivery
							data = data.content;
						}else{
...
/*--------------------------User Search--------------------------*/
	...
	$.ajax({
		url: HOST_URL + "/delivery/products/?currentPage="+currentPage,
		type: "GET",
		success:function(data) {
			var productCount = data.totalElements;
			data = data.content;
...
function statusUpdate(){
			    $('.statusUpdate').click(function (event) {
			        ...
			        $.ajax({
			        	url: HOST_URL + "/delivery/product/status/"+id+"/",
						type: "PUT",
			            data: ser_data,
...
function productDelete(){
			...
			function productDelete(silinecekId){
				$(function() {
					...
					if (result.value) {
						$.ajax({
						url: HOST_URL + "/delivery/product/"+silinecekId+"/",
						type: "DELETE",
					    contentType: "application/json",
...
/*--------------------------Product Add--------------------------*/
			    ...
			    $('#productSave').click(function (event) {
			        ...
			        $.ajax({
			        	url: HOST_URL + "/delivery/product",
						type: "POST",
			            data: ser_data,
					    contentType: "application/json",
			            success: function (result) {
			                toastr.success('Ürün Eklendi.', 'İşlem Başarılı', {"progressBar": true,positionClass: "toast-bottom-right",timeOut: 1500})
							setTimeout(function(){   
...
/*--------------------------!Product Add Category View--------------------------*/
			    ...
			    $('.categorySelect').on('change', function() {
				  ...
				  var value = $(this).val(); 
				  	$.ajax({
					url: HOST_URL + "/delivery/subcategory"+value,
					type: "GET",
					success:function(data) {
						data = data;
